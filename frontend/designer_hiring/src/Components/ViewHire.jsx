...
function ViewHire() {
    ...
    useEffect(() => {
        const fetchHiredDetails = async () => {
            try {
                const response = await ApiRequest.get('/hire/getdesigner');
                const hiredData = response.data.data;
    ...
    return (
        <>
            ...
            <h1>Hired Details</h1>
            {error && <p>{error}</p>}
            {hiredDetails.length > 0 ? (
                <ul>
                    {hiredDetails.map((detail) => (
                        <li key={detail._id}>
                            <h2>User: {userDetails[detail.userId]?.username || detail.userId} Hired YOU </h2>
                            {/* <h2>Designer: {designerDetails[detail.desgnerId]?.username || detail.desgnerId}</h2> */}
                        </li>
                    ))}
                </ul>
            ) : (
